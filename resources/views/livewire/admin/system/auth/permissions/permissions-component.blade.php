<div>
    <div class="page-header">
        <h3 class="fw-bold mb-3">Permisos</h3>
        <ul class="breadcrumbs mb-3">
            ...
            <li class="nav-item">
                <a href="{{ route('admin.system.auth.permissions.index') }}">Permisos</a>
            </li>
        </ul>
    </div>
    ...
    <div class="card">
                ...
                <div class="col-sm-12 col-md-6 dflex justify-content-md-start">
                    <div class="form-group">
                        <div class="input-icon">
                            <input type="text" class="form-control" placeholder="Buscar permisos..."
                ...
                <div class="col-sm-12 col-md-6 d-flex justify-content-md-end">
                    @permission('create-permissions')
                        <button class="btn btn-primary btn-round" wire:click="create()">
                            <span class="fas fa-plus me-2"></span> Agregar permiso
                        </button>
                    @endpermission
                    @if ($isOpen)
                        @include('livewire.admin.system.auth.permissions.permission-modal')
                    @endif
                </div>
            </div>
        </div>
        <div class="card-body">
                    ...
                    <tbody>
                        @forelse ($permissions as $index => $permission)
                            <tr>
                                ...
                                <td>
                                    <p class="mb-0 text-body-emphasis">{{ $permission->description }}</p>
                                </td>
                                <td class="d-flex align-items-center">
                                    @permission('update-permissions')
                                        <button class="btn btn-info btn-sm me-1" wire:click="edit({{ $permission->id }})">
            ...
            <div class="mt-3 mb-3 d-flex justify-content-center">
                <a class="fw-semibold" href="{{ route('admin.system.auth.permissions.index') }}" data-list-view="*">Ver
                    todo
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            window.Livewire.on('confirmDeletePermission', permissionId => {
                Swal.fire({
                    title: "¿Estás seguro?",
                    ...
                    if (result.isConfirmed) {
                        window.Livewire.dispatch('deletePermission',
                        permissionId); // Enviamos solo el ID
                    }
                });
            });
        });
